				...
				const searchSetting = new Setting(row).addSearch((cb) => {
					...
					cb.setPlaceholder("Folder")
						...
						.onChange(async (newFolder) => {
							this.plugin.settings.excluded_folder[index].folder =
								newFolder;
...
export class AutoNoteMoverSettingTab extends PluginSettingTab {
	...
	add_auto_note_mover_setting(): void {
		...
		new Setting(this.containerEl)
			...
			.addDropdown((dropdown) => {
				dropdown
					...
					.setValue(
						this.plugin.settings.duplicate_file_action || "skip",
					)
					.onChange(async (value) => {
		...
		new Setting(this.containerEl)
			...
			.addToggle((toggle) => {
				toggle
					...
					.onChange(async (value) => {
						this.plugin.settings.use_regex_to_check_for_tags =
							value;
						await this.plugin.saveSettings();
						this.display();
					});
		...
		this.plugin.settings.folder_tag_pattern.forEach((rule, index) => {
			...
			const renderConditionsList = (listEl: HTMLElement) => {
				...
				rule.conditions.forEach((cond, condIndex) => {
					...
					["tag", "title", "property", "date", "folder"].forEach(
						(opt) => {
					...
					["frontmatter", "metadata"].forEach((opt) => {
						...
						o.value = opt;
						o.text =
							opt === "frontmatter" ? "Frontmatter" : "Metadata";
					...
					["ctime", "mtime"].forEach((opt) => {
						...
						o.value = opt;
						o.text =
							opt === "ctime" ? "Created time" : "Modified time";
					...
					const subfolderSelect =
						folderOptionsWrap.createEl("select");
					["include", "exclude"].forEach((opt) => {
						...
						o.text =
							opt === "include"
								...
								: "Exclude subfolders";
						o.selected =
							(cond.includeSubfolders ? "include" : "exclude") ===
							opt;
					});

					subfolderSelect.onchange = async () => {
						cond.includeSubfolders =
							subfolderSelect.value === "include";
						await this.plugin.saveSettings();
					};
					...
					metadataSelect.onchange = async () => {
						cond.metadataField =
							metadataSelect.value as MetadataField;
						await this.plugin.saveSettings();
					};
			...
			toggleBtn.onClick(async () => {
				...
				toggleBtn.setIcon(
					rule.collapsed ? "chevron-right" : "chevron-down",
				);
			});
			...
			const matchSetting = new Setting(headerMain).addDropdown((drop) => {
				drop.addOption("ALL", "Match if all")
		...
		const useRegexToCheckForExcludedFolder =
		...
		new Setting(this.containerEl)
			...
			.addToggle((toggle) => {
				toggle
					...
					.onChange(async (value) => {
						this.plugin.settings.statusBar_trigger_indicator =
							value;
						await this.plugin.saveSettings();
						this.display();
					});
